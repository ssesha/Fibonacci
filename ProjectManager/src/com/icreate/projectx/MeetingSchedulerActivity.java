				...
				try {
					...
					if (e instanceof HttpResponseException) {
						...
						if (statusCode == 401 && (tryNumber - 1) > 0) {
							...
							OAuthManager.getInstance().doLogin(true, MeetingSchedulerActivity.this, new OAuthManager.AuthHandler() {
					...
					if (done) {
						handler.post(new Runnable() {
							@Override
							public void run() {
								Toast.makeText(MeetingSchedulerActivity.this, getString(R.string.available_time_retrieval_failure) + ": " + e.getMessage(), Toast.LENGTH_LONG).show();
							}
						});
					}
				}
	...
	private void populateMeetingTimes(List<AvailableMeetingTime> newTimes) {
		...
		meetingListContainer.setAdapter(new EventExpandableListAdapter(this, availableMeetingTimes, meetingLength, new EventExpandableListAdapter.EventHandler() {
			...
			public void handleEventSelected(long startTime, long endTime) {
				try {
					startActivityForResult(CreateEventActivity.createViewIntent(MeetingSchedulerActivity.this, selectedAttendees, startTime, endTime), Constants.CREATE_EVENT);
